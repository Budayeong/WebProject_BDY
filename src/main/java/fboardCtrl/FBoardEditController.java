...
@WebServlet("/board/fboardEdit.do")
@MultipartConfig(
		maxFileSize = 1024 * 1024 *100,
		maxRequestSize = 1024 * 1024 * 1000
		)
public class FBoardEditController extends HttpServlet {
...
//		DB에 수정 내용 적용
		...
		if(result==1) {
			resp.sendRedirect("../board/fboardView.do?num=" + num);
		}
		else
//			실패하면 경고창을 띄우고 이동
			JSFunction.alertLocation(resp, "비밀번호 검증을 다시 진행해주세요.", "../board/fboardView.do?num=" +num);
	}
}
